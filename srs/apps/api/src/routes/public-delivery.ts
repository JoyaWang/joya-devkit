 ...
 * - dl-dev.infinex.cn
 * - dl.infinex.cn
 * - srs.infinex.cn (via /public/* prefix)
 *
 * Behavior:
 * - only active + public-stable objects are eligible
 * - host/env must match (dl-dev => dev/staging, dl/srs => prod; prd kept for backward compat)
...
function isAllowedHost(hostname: string): boolean {
  return hostname === "dl-dev.infinex.cn" || hostname === "dl.infinex.cn" || hostname === "srs.infinex.cn";
}

function hostMatchesEnv(hostname: string, objectEnv: string): boolean {
  if (hostname === "dl.infinex.cn" || hostname === "srs.infinex.cn") {
...
export async function registerPublicDeliveryRoute(
  ...
  // Register /public/* route for srs.infinex.cn (strips /public/ prefix)
  app.get(
    "/public/*",
    {
      constraints: {
    ...
    async (
      ...
      // Strip "public/" prefix — the wildcard captures "infov/prod/public/feedback/..."
      let objectKey = request.params["*"];
      if (!objectKey) {
        return reply.status(400).send({ error: 'field "objectKey" is required' });
      }

      // Handle case where wildcard already excludes the "public/" prefix
      if (objectKey.startsWith("public/")) {
        objectKey = objectKey.slice(7);
      }

      return handlePublicDelivery(objectKey, hostname, request, reply, deps);
    },
  );

  // Register catch-all route for dl-dev.infinex.cn and dl.infinex.cn
  app.get(
    "/*",
    {
      constraints: {
