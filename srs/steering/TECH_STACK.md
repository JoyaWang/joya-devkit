...
- 中期推荐收敛为“两个共享 bucket + 两个共享 origin 域名”模型：`shared-dev-bucket` / `shared-prd-bucket` 承载对象本体，`origin-dev.infinex.cn` / `origin.infinex.cn` 承载共享真实下载出口；项目之间继续通过 objectKey 前缀隔离
