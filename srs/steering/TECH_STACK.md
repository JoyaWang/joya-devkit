...
**前提**：CNB CLI 已登录（`~/.joya/vault/joya-vault-run bash -c 'cnb auth login --token "$CNB_TOKEN"'`）。

#### 1. API 触发（命令行）

```bash
# dev 部署
~/.joya/vault/joya-vault-run bash -c 'curl -s -X POST \
  -H "Authorization: Bearer $CNB_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"event\":\"api_trigger_deploy_dev\"}" \
  "https://api.cnb.cool/joyawang/joya-devkit/-/build/start"'

# prod 部署
~/.joya/vault/joya-vault-run bash -c 'curl -s -X POST \
  -H "Authorization: Bearer $CNB_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"event\":\"api_trigger_deploy_prod\"}" \
  "https://api.cnb.cool/joyawang/joya-devkit/-/build/start"'
```

#### 2. Web Trigger（浏览器按钮）

打开 CNB 仓库页面 → 构建 → Web Trigger 按钮：
- dev: "Build SRS api/worker on CNB, push TCR, deploy dev server"
- prod: "Build SRS api/worker on CNB, push TCR, deploy prod server"

#### 3. Push 自动触发

dev 分支 push 自动触发 dev 部署流水线。

#### 查看构建状态

```bash
# 查看最近构建列表
~/.joya/vault/joya-vault-run bash -c 'CNB_TOKEN="$CNB_TOKEN" cnb build get-build-logs --repo joyawang/joya-devkit --page-size 5'

# 查看指定构建状态（替换 <SN>）
~/.joya/vault/joya-vault-run bash -c 'CNB_TOKEN="$CNB_TOKEN" cnb build get-build-status --repo joyawang/joya-devkit --sn <SN>'
```

#### 服务器路由

dev 和 prod 部署到不同服务器。区分靠 Vault token 环境：
- `INFISICAL_SERVICE_TOKEN_INFRA_DEV` → 从 Vault 拉取 dev 服务器凭据
- `INFISICAL_SERVICE_TOKEN_INFRA_PROD` → 从 Vault 拉取 prod 服务器凭据

脚本中统一使用 `SHARED_SERVER_*` 变量，由 `fetch_server_credentials` 根据环境自动选择正确 token。
