...
# --- O-01: Upload request (infov) ---
echo ""
echo "[O-01] POST /v1/objects/upload-requests (infov)"
STATUS=$(curl -s -o /tmp/e2e-upload.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{
    \"project\": \"$PROJECT\", \"env\": \"prod\", \"domain\": \"member\",
    \"scope\": \"avatar\", \"entityId\": \"user_e2e\",
    \"fileKind\": \"profile\", \"fileName\": \"e2e.png\",
    \"contentType\": \"image/png\", \"size\": 5120
  }")
BODY=$(cat /tmp/e2e-upload.json)
assert_status "upload request returns 201" 201 "$STATUS"
assert_has_field "upload response has objectKey" "$BODY" "objectKey"
assert_has_field "upload response has uploadUrl" "$BODY" "uploadUrl"
assert_has_field "upload response has expiresAt" "$BODY" "expiresAt"
assert_field_contains "objectKey format correct" "$BODY" "objectKey" "$PROJECT/prod/member/avatar/user_e2e"

OBJECT_KEY=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['objectKey'])" 2>/dev/null)

# --- O-01b: Project mismatch ---
echo ""
echo "[O-01b] POST /v1/objects/upload-requests (project mismatch)"
STATUS=$(curl -s -o /tmp/e2e-mismatch.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
...
BODY=$(cat /tmp/e2e-mismatch.json)
...
# --- O-01c: laicai upload (cross-project, consistent) ---
echo ""
echo "[O-01c] POST /v1/objects/upload-requests (laicai)"
STATUS=$(curl -s -o /tmp/e2e-cross-project.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
  -H "Authorization: Bearer dev-token-laicai" -H "Content-Type: application/json" \
  -d "{
    \"project\": \"laicai\", \"env\": \"prod\", \"domain\": \"member\",
    \"scope\": \"avatar\", \"entityId\": \"user_cross\",
    \"fileKind\": \"profile\", \"fileName\": \"cross.png\",
    \"contentType\": \"image/png\", \"size\": 2048
  }")
BODY=$(cat /tmp/e2e-cross-project.json)
assert_status "laicai consistent upload returns 201" 201 "$STATUS"
assert_field_contains "objectKey starts with laicai" "$BODY" "objectKey" "laicai/prod/member/avatar/user_cross"
assert_field_contains "uploadUrl hits laicai bucket" "$BODY" "uploadUrl" "laicai-dev-bucket"

LAICAI_OBJECT_KEY=$(echo "$BODY" | python3 -c "import sys,json; print(json.load(sys.stdin)['objectKey'])" 2>/dev/null)

# --- O-01d: infov upload URL hits infov bucket ---
echo ""
echo "[O-01d] Upload URL hits correct project bucket (infov)"
INFOV_UPLOAD_URL=$(cat /tmp/e2e-upload.json | python3 -c "import sys,json; print(json.load(sys.stdin).get('uploadUrl',''))" 2>/dev/null)
if echo "$INFOV_UPLOAD_URL" | grep -q "infov-dev-bucket"; then
  echo "  PASS: infov upload URL contains infov-dev-bucket"
  PASS=$((PASS + 1))
else
  echo "  FAIL: infov upload URL does not contain infov-dev-bucket (got: $INFOV_UPLOAD_URL)"
  FAIL=$((FAIL + 1))
fi

# --- O-01e: Unbound project returns service_binding_missing ---
echo ""
echo "[O-01e] POST /v1/objects/upload-requests (unbound project)"
STATUS=$(curl -s -o /tmp/e2e-unbound.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
  ...
  -d "{
    \"project\": \"unbound\", \"env\": \"prod\", \"domain\": \"member\",
...
# --- O-01f: Unregistered project returns project_not_registered ---
echo ""
echo "[O-01f] POST /v1/objects/upload-requests (ghost — not registered)"
STATUS=$(curl -s -o /tmp/e2e-ghost.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
  ...
  -d "{
    \"project\": \"ghost\", \"env\": \"prod\", \"domain\": \"member\",
...
STATUS=$(curl -s -o /tmp/e2e-scope.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/upload-requests" \
  ...
  -d "{
    \"project\": \"$PROJECT\", \"env\": \"prod\", \"domain\": \"member\",
    \"scope\": \"evil_scope\", \"entityId\": \"user_e2e\",
...
# --- O-03: Complete upload ---
echo ""
echo "[O-03] POST /v1/objects/complete"
STATUS=$(curl -s -o /tmp/e2e-complete.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/complete" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{\"objectKey\": \"$OBJECT_KEY\", \"size\": 5120, \"checksum\": \"e2e-sha256\"}")
BODY=$(cat /tmp/e2e-complete.json)
assert_status "complete returns 200" 200 "$STATUS"
assert_field_eq "complete status=active" "$BODY" "status" "active"

# --- O-04: Download request ---
echo ""
echo "[O-04] POST /v1/objects/download-requests"
STATUS=$(curl -s -o /tmp/e2e-download.json -w "%{http_code}" -X POST "$BASE_URL/v1/objects/download-requests" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{\"objectKey\": \"$OBJECT_KEY\"}")
BODY=$(cat /tmp/e2e-download.json)
assert_status "download request returns 200" 200 "$STATUS"
assert_has_field "download response has downloadUrl" "$BODY" "downloadUrl"
assert_has_field "download response has expiresAt" "$BODY" "expiresAt"
assert_field_contains "download URL hits infov bucket" "$BODY" "downloadUrl" "infov-dev-bucket"

# --- O-05: Delete object ---
echo ""
echo "[O-05] DELETE /v1/objects"
STATUS=$(curl -s -o /tmp/e2e-delete.json -w "%{http_code}" -X DELETE "$BASE_URL/v1/objects" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d "{\"objectKey\": \"$OBJECT_KEY\"}")
BODY=$(cat /tmp/e2e-delete.json)
assert_status "delete returns 200" 200 "$STATUS"
assert_field_eq "delete status=deleted" "$BODY" "status" "deleted"
