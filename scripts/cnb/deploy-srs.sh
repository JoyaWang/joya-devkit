...
fetch_server_credentials "$SERVER_EXPORTS"
# shellcheck disable=SC1090
source "$SERVER_EXPORTS"

case "$DEPLOY_ENV" in
  dev)
    SERVER_HOST="${DEV_SERVER_HOST:-${DEV_SERVER_IP:-${SERVER_HOST:-${SERVER_IP:-${SHARED_SERVER_IP:-}}}}}"
    SERVER_USER_VALUE="${DEV_SERVER_USER:-${SERVER_USER:-${SHARED_SERVER_USER:-}}}"
    SERVER_PORT_VALUE="${DEV_SERVER_PORT:-${SERVER_PORT:-${SHARED_SERVER_PORT:-22}}}"
    SERVER_KEY_VALUE="${DEV_SERVER_SSH_KEY:-${SERVER_SSH_KEY:-${SHARED_SERVER_SSH_KEY:-}}}"
    SERVER_PASS_VALUE="${DEV_SERVER_PASS:-${SERVER_PASS:-${SHARED_SERVER_PASS:-}}}"
    ;;
  prod)
    SERVER_HOST="${PROD_SERVER_HOST:-${PROD_SERVER_IP:-${SERVER_HOST:-${SERVER_IP:-${SHARED_SERVER_IP:-}}}}}"
    SERVER_USER_VALUE="${PROD_SERVER_USER:-${SERVER_USER:-${SHARED_SERVER_USER:-}}}"
    SERVER_PORT_VALUE="${PROD_SERVER_PORT:-${SERVER_PORT:-${SHARED_SERVER_PORT:-22}}}"
    SERVER_KEY_VALUE="${PROD_SERVER_SSH_KEY:-${SERVER_SSH_KEY:-${SHARED_SERVER_SSH_KEY:-}}}"
    SERVER_PASS_VALUE="${PROD_SERVER_PASS:-${SERVER_PASS:-${SHARED_SERVER_PASS:-}}}"
    ;;
  *)
    fail "DEPLOY_ENV must be dev or prod"
    ;;
esac
